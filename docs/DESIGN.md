...
- 다크: 푸른 검정이 아니라 갈색 검정(`#17140F`/`#221E18`), 초록은 밝게(`#4FBF85`)라 그 위 글자(`onPrimary`)는 어두운 색.

## 토큰 (`src/theme.ts` 만이 출처)
| 구분 | 토큰 | 값 (light / dark) |
| --- | --- | --- |
| 색 | `bg` / `card` | `#FAF6EF`(크림) `#FFFFFF` / `#17140F` `#221E18`(갈색 검정) |
| | `text` / `textMuted` | `#2B2520` `#6F645A` / `#F2EDE4` `#A99D8F` — textMuted 는 bg 위 5.35 · card 위 5.76 / card 위 6.24 (진단서의 `#7A6F64` 는 bg 위 4.55 경계라 한 단계 진하게) |
| | `primary` / `onPrimary` / `primarySoft` | `#23794B`(숲 초록) `#FFFFFF` `#E3F1E8` / `#4FBF85` `#17140F` `#1D3326` — primary 는 card 위 5.37 · bg 위 4.99 / card 위 7.20. 다크 primary 위 흰 글자는 2.3 이라 onPrimary 를 어두운 색(7.98)으로 |
| | `onPrimarySoft`(primarySoft 위 글자·아이콘) | `#1A6B40` / `#6FD49E` (5.59 / 7.46) |
| | `danger` | `#CE2C31` / `#F2555A` (card 위 5.21 / 4.91, 스와이프 삭제 면 위 onPrimary 5.21 / 5.44) |
| | `border`(컨트롤 테두리) / `divider`(카드 안 구분선) | `#EAE2D6` `#F1EBE1` / `#3A3229` `#342D24` (다크 구분선은 카드보다 밝게) |
| | `grass1~4`(잔디 농도) / `grassFuture`(잔디 미래 칸) | `#B6DCC1` `#7FC096` `#4C9E70` `#23794B` `#FAF6EF` / `#1F4A31` `#2B6B46` `#3C9563` `#4FBF85` `#17140F` — 새싹 → 숲 초록(4 = primary). 1단계는 기록 없는 날 `divider` 와 1.27 / 1.35 로 구분, 미래 칸은 `bg` 와 같은 움푹한 칸 |
| | `todayRing`(잔디 오늘 칸 테두리) | `#2B2520` / `#F2EDE4` (글자 톤). primary 였을 땐 4단계 칸(= primary)에서 사라졌다. 칸 안쪽 grass4 와는 2.8 / 2.0 이지만 바깥 card 와 15 / 14 라 테두리 모양으로 읽힌다 — 알려진 절충 |
| | `warn` / `onWarn`(주황 면과 그 위 글자) | `#F28C28` `#2B2520` / `#FFA24C` `#17140F` (6.16 / 9.2). **면 전용** — 🔥 칩 바탕·컨페티. 면 자체는 bg 와 2.3 이라 글자는 반드시 onWarn |
| | `warnSoft`(목표 달성 카드 틴트) | `#FDEBD6` / `#3A2A18` (그 위 primary 4.61 / 5.99, textMuted 4.94 / 5.19) |
| | `good`(컨페티 조각 전용, 버터 노랑) | `#EDB83D` / `#F2C94C` — 메인 초록과 겹치지 않게 초록에서 노랑으로 바꿈. 글자·넓은 면에 쓰지 않음 |
...
- `primary` 는 다크에서 한 톤 밝게(`#4FBF85`). 밝은 초록 위 흰 글자는 대비가 모자라 다크 `onPrimary` 는 어두운 `#17140F`(FAB 아이콘·선택 칩 글자·스와이프 삭제 글자 모두). 배경은 갈색 검정 — 순수 검정·순수 흰색 배경은 쓰지 않는다.
