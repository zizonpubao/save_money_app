...
## 분위기
매일 열고 싶은, 차분하지만 뿌듯한 앱. 화면의 주인공은 딱 하나 — 이번 달 절약액이다. 그 숫자만 크고 초록으로 두고 나머지(날짜·항목·합계)는 갈색 톤 회색으로 뒤로 물린다. 가계부처럼 표가 빽빽하지도, 게임처럼 뱃지가 번쩍이지도 않게: 크림 바탕 위 흰 카드 + 얇은 구분선으로 차분함을 지키고, 기쁨은 저장 순간의 짧은 카운트업·펄스·햅틱에만 담는다. 여백은 넉넉히, 색은 아껴서. 기록이 없는 달도 실패처럼 보이지 않게 한다.

## 방향 — B "따뜻한 저금통" (사용자 결정, 파란 primary 를 대신함)
- 콘셉트: 돈이 조금씩 쌓이는 크림색 저금통. 은행 앱의 파랑 대신 크림 바탕 `bg` 위 흰 카드, 주인공 숫자와 잔디는 숲 초록 `primary`.
- 색 역할: 초록 = 평소의 강조(숫자·FAB·선택·활성 탭·잔디·막대·목표 바), 주황 `warn` = 성취의 면(🔥 칩 바탕·컨페티·목표 달성 틴트 `warnSoft`)에만. 주황은 글자색으로 쓰지 않는다(크림 위 2.3).
- 형태: 둥근 모서리 — 카드 `radius.lg` 22, 입력·버튼·행 묶음 `radius.md` 16, 칩 pill, FAB 원형. 그림자는 여전히 떠 있는 것에만.
- 숫자: 금액·숫자 텍스트에만 라틴 폰트 **Outfit**(tnum 지원, 굵기 400/600/700/800 각 ≈54KB). 한글은 시스템 폰트 그대로 — "원"·"누적" 같은 한글은 자동으로 시스템 폰트로 대체된다.
- 다크: **중성 차콜 다크 + 생생한 초록·주황**. 갈색 기운 없는 아주 살짝 차가운 차콜(`#131517`/`#1E2125`) 위에 초록은 밝고 생생하게(`#3DD68C`), 주황은 `#FF9F43`. 밝은 초록 위 글자(`onPrimary`)는 어두운 색. 라이트(크림·갈색 톤 글자)는 그대로.

## 토큰 (`src/theme.ts` 만이 출처)
| 구분 | 토큰 | 값 (light / dark) |
| --- | --- | --- |
| 색 | `bg` / `card` | `#FAF6EF`(크림) `#FFFFFF` / `#131517` `#1E2125`(중성 차콜, 1.13) |
| | `text` / `textMuted` | `#2B2520` `#6F645A` / `#F2F3F4` `#9BA1A8` — textMuted 는 bg 위 5.35 · card 위 5.76 / card 위 6.20 · bg 위 7.02 (진단서의 `#7A6F64` 는 bg 위 4.55 경계라 한 단계 진하게) |
| | `primary` / `onPrimary` / `primarySoft` | `#23794B`(숲 초록) `#FFFFFF` `#E3F1E8` / `#3DD68C` `#131517` `#163526` — primary 는 card 위 5.37 · bg 위 4.99 / card 위 8.62. 다크 primary 위 흰 글자는 대비가 모자라 onPrimary 를 어두운 색(9.76)으로 |
| | `onPrimarySoft`(primarySoft 위 글자·아이콘) | `#1A6B40` / `#6BE3A6` (5.59 / 8.38) |
| | `danger` | `#CE2C31` / `#FF5C61` (card 위 5.21 / 5.35, 스와이프 삭제 면 위 onPrimary 5.21 / 6.06) |
| | `border`(컨트롤 테두리) / `divider`(카드 안 구분선) | `#EAE2D6` `#F1EBE1` / `#393E45` `#2E3237` (다크 구분선은 카드보다 밝게, 중성 회색) |
| | `grass1~4`(잔디 농도) / `grassFuture`(잔디 미래 칸) | `#B6DCC1` `#7FC096` `#4C9E70` `#23794B` `#FAF6EF` / `#1A5236` `#227A4E` `#2BA366` `#3DD68C` `#131517` — 새싹 → 숲 초록(4 = primary). 1단계는 기록 없는 날 `divider` 와 1.27 / 1.42 로 구분(다크 단계 사이 1.65~1.72), 미래 칸은 `bg` 와 같은 움푹한 칸 |
| | `todayRing`(잔디 오늘 칸 테두리) | `#2B2520` / `#F2F3F4` (글자 톤). primary 였을 땐 4단계 칸(= primary)에서 사라졌다. 칸 안쪽 grass4 와는 2.8 / 1.7 이지만 바깥 card 와 15 / 14.6 이라 테두리 모양으로 읽힌다 — 알려진 절충 |
| | `warn` / `onWarn`(주황 면과 그 위 글자) | `#F28C28` `#2B2520` / `#FF9F43` `#131517` (6.16 / 8.97). **면 전용** — 🔥 칩 바탕·컨페티. 면 자체는 bg 와 2.3 이라 글자는 반드시 onWarn |
| | `warnSoft`(목표 달성 카드 틴트) | `#FDEBD6` / `#42290F` (그 위 primary 4.61 / 7.20, textMuted 4.94 / 5.18) |
| | `good`(컨페티 조각 전용, 버터 노랑) | `#EDB83D` / `#F5CB4A` — 메인 초록과 겹치지 않게 초록에서 노랑으로 바꿈. 글자·넓은 면에 쓰지 않음 |
| | `shadow`(그림자 색 전용) | `#2B2520` / `#000000` (다크에서 `text` 를 쓰면 흰 번짐) |
| 간격 | `sp.xs/sm/smd/md/lg/xl` | 4 / 8 / 12 / 16 / 24 / 32 (4의 배수만) |
| 타이포 | `type.display` | 40 / 800 / 52 · **Outfit ExtraBold** — 이번 달 절약액 (숫자 전용) |
| | `type.title` | 28 / 700 / 36 · **Outfit Bold** — 금액 입력값 (숫자 전용. 행간은 Outfit 글자 높이 1.26em 보다 크게) |
...
| | `numFont` / `numFace.regular/semibold/bold/extrabold` | Outfit 400·600·700·800. 금액·숫자 텍스트에만 `[type.x, numeric, numFace.<같은 굵기>]` 로 덧붙인다. fontWeight 도 같이 들어 있어 폰트 로드 실패 때 시스템 폰트가 같은 굵기로 그린다 |
| radius | `radius.xs/sm/md/lg/pill` | 4(잔디 칸) / 12(세그먼트 안 칸 = md − sp.xs) / 16(입력·버튼·행 묶음·배너) / 22(카드) / 999(칩) |
...
첫 화면(390×844, 헤더·탭바 제외 ≈670pt)에 **목록 3행 이상**이 보이도록 머리 영역을 ≈470pt 안에 둔다 (카드 ≈260 — B 에서 큰 숫자 행간 42 → 52 · 칩 32 · 잔디 5줄 140 / 6줄 160).
...
- (B) 숫자 폰트 Outfit 은 금액·숫자가 주인공인 텍스트에만: 큰 숫자(`AnimatedWon`·통계 카드), 카드 "오늘" 줄, 목록·원탭 메뉴 행 금액, 섹션 합계, 통계 보조 줄, 카테고리 금액·%, 정보 칩, 목표 %, 막대 툴팁, 플로팅 라벨, 금액 입력값. 한글 위주 문장(오늘의 한 줄·배너·회고·설정 값·금액 프리셋 칩 "5천")은 시스템 폰트 그대로 — `numeric` 에는 폰트를 넣지 않는다.
- 큰 숫자일수록 굵게: 절약액 `display`(800) > 행 금액 `bodyStrong`(600) > 섹션 합계 `note`(400).
- 날짜는 `2026년 9월 23일 (수)`, 월은 `2026년 9월`. 상대 표현("어제")은 쓰지 않는다.

## 다크모드
- 기본은 시스템 설정을 따르고, 설정 → 화면에서 시스템·라이트·다크를 고를 수 있다(Appearance.setColorScheme). 색은 `useTheme()` 에서만 가져온다.
- `bg` < `card` 순서로 카드가 항상 한 단계 밝다. 다크에서 구분선은 카드보다 밝은 `divider` 로.
- `primary` 는 다크에서 밝고 생생하게(`#3DD68C`). 밝은 초록 위 흰 글자는 대비가 모자라 다크 `onPrimary` 는 어두운 `#131517`(FAB 아이콘·선택 칩 글자·스와이프 삭제 글자 모두). 배경은 중성 차콜 — 갈색 기운·순수 검정·순수 흰색 배경은 쓰지 않는다.
...
| 120 | `FloatingLabel` | `display`(40/800 Outfit), "+55,000원 🔥", 위로 **60pt**, scale **0.5 → 1.4 → 1**, **880ms** (900 이면 1020 에 끝나 1초 상한을 넘어 20ms 줄임) |
...
- **목표 달성**: t0+280 바가 100% 까지 `withSpring {damping 11, stiffness 220}` · 카드 틴트 `warnSoft`(옅은 주황, B) 0 → 1 → 0, t0+80(타격과 함께) 시작 → 880 끝 · 컨페티 색 비율 `good`(버터 노랑) 50% / `primary`(숲 초록) 25% / `warn`(주황) 25% · 소리 `fanfare.wav`(hit 대신) · 햅틱은 big 과 같음(Heavy 3연타 + t0+500 Success 1회 — Success 가 두 번 울리지 않게) · 배너 "이번 달 목표 달성 🎉" 가 t0+300 에 **위에서 튕겨 내려옴** translateY −20 → 0 `withSpring {damping 11, stiffness 260}` + opacity 0 → 1 120ms.
...
- **컨페티 팔레트 (B)**: 자리 0~3 = `primary`(숲 초록) · `grass2`(새싹 연두) · `good`(버터 노랑) · `warn`(주황). 기본은 4색 고르게, 목표 달성은 노랑 50% · 초록 25% · 주황 25%. 크림·흰 바탕에서 안 보이는 옅은 색(`primarySoft`)과 흙처럼 보이는 갈색은 조각에 쓰지 않는다.
...
- (B) `warn` 주황을 글자색·선으로 쓰기, 평소 상태(선택·탭·버튼)에 주황 쓰기 — 주황은 성취의 면에만. 한글 위주 문장에 숫자 폰트 넣기.
...
- **(B 방향 "따뜻한 저금통")**: 색 토큰 전면 교체(파랑 → 크림·숲 초록·갈색 톤, 새 토큰 `onWarn`·`warnSoft`·`todayRing`), 다크 `onPrimary` 를 어두운 글자로. `radius.lg` 16 → 22 · `md` 12 → 16 · `sm` 8 → 12. 숫자 폰트 Outfit 4굵기(`@expo-google-fonts/outfit` 굵기별 경로만 로드, `app/_layout.tsx` 에서 DB 준비와 함께 스플래시 게이트) + `numFace` 토큰, 큰 숫자 36 → 40. 🔥 칩 주황 면(`StatChip tone="warn"`), 목표 달성 틴트 `warnSoft`, 잔디 오늘 테두리 `todayRing`, 컨페티 2번 자리 `primarySoft` → `grass2`. 대비 쌍은 `themeContrast.test.ts` 가 두 모드에서 지킨다.
- **다크 차콜 (사용자: 갈색 다크가 썩은 나무 느낌 → 차콜로)**: 다크 팔레트만 갈색 검정 → 중성 차콜(`bg #131517` · `card #1E2125` · 회색 `border`/`divider`/`textMuted`), `primary`·잔디·`onPrimarySoft` 를 생생한 초록(`#3DD68C` 계열), `warn #FF9F43` · `danger #FF5C61` · `good #F5CB4A`. 라이트는 그대로.
