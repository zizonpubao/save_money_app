...
/** 기간 단위. 홈 목록은 항상 'month', 월/년 토글은 기록 탭(useMonthlySummary)에서 쓴다. */
...
function fetchSnapshot(oldestMonth: string) {
  const range = computeRange('month', oldestMonth);
...
export const useEntryStore = create<EntryState>((set, get) => ({
  ...
  add: (input) => {
    ...
    const { oldestMonth, celebrateTick } = get();
    set({ ...fetchSnapshot(oldestMonth), celebrateTick: celebrateTick + 1, lastRecord });
    return created;
  },
  ...
  update: (id, input) => {
    ...
    set({ ...fetchSnapshot(get().oldestMonth), lastRecord: null });
    return updated;
  },
