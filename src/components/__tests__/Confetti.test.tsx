...
describe('Confetti (M4)', () => {
  ...
  it('두 번 터지면 24 + 24 = 48개 (big)', async () => {
    await render(<Confetti bursts={2} perBurst={24} seed={2} />);
  ...
  it('동작 줄이기가 켜져 있으면 그리지 않는다', async () => {
    await render(<Confetti bursts={2} perBurst={24} seed={1} reduceMotion />);
    expect(screen.queryAllByTestId('confetti-piece')).toHaveLength(0);
  });

  it('목표 달성 팔레트면 good 색 조각이 가장 많다', async () => {
    await render(<Confetti bursts={2} perBurst={24} seed={5} palette="goal" />);
    const colors = screen
