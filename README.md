...
| # | 기능 | 요약 |
|---|---|---|
| 1 | 기록 입력 | 금액(자동 콤마) · 항목 · 카테고리 칩(9개) · 빠른 입력 칩 · 날짜 · 메모. `+` 길게 누르기로 최근 항목 원탭 저장, 금액 프리셋 칩(500원·1천·3천·5천·1만, 누적). 저장 시 햅틱 + 효과음 + 금액 카운트업 + 카드 타격, 4천원 이상 컨페티 · 2만원 이상 세 번 터짐 + 플래시 + 화면 흔들림 |
...
적용된 마이그레이션은 고치지 않고 새 버전을 덧붙입니다 (v1 초기 스키마 → v2 밥값 카테고리 → v3 settings → v4 옷 카테고리 → v5 옷을 기본에서 제거).
